...
function fn_getCategory2(){
	
	if ( $("#category1").val() != ""){
		
		$.ajax({
			type : "post",		
			url : "./selectCategpry.json",
			dataType : 'json',
			//data: $('#frm').serialize() ,
			data:{
				parent_code : $("#category1").val()
			},
			success : function(data) {
				...
				if ( data.success){
					...
					$.each(data.list, function(i, item) {
						var selected = "";
						if ( item.code_id == "${detail.category2}" ){
							selected = "selected";
						}
						$("#category2").append("<option value='"+item.code_id+"'  "+selected+" >"+item.code_name+"</option>");
					});			
...
<body>
	...
	<div class="contWarp">
		...
		<div class="contents">
			...
			<input type="hidden" name="contents_gubun" id="contents_gubun" value="C2"/>
						...
						<tr>
							...
							<td>
								<select id="category1" name="category1" >
									...
									<c:forEach var="list" items="${category1List}" varStatus="status">
										<option value="${list.code_id }"   ${ detail.category1 == list.code_id ? "selected" : ""  }  >${list.code_name }</option>
									</c:forEach>
