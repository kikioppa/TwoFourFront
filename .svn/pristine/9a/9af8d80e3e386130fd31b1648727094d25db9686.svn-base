	...
	<div class="container faq">
		...
		<form name="frm" id="frm" method="post">
			...
			<div class="input-text-wrap">
				<input type="text" name="searchKeyword" id="searchKeyword" value="<c:out value='${param.searchKeyword}'/>" placeholder="검색어를 입력해주세요">
			...
			<div class="quick-tag">
				<ul>
					<c:forEach items="${categoryList}" var="list" varStatus="status">
						<li <c:if test="${listHelper.list[0].CATE_SEQ == list.SEQ}">class="on"</c:if>><a href="javascript:cate_change(<c:out value='${list.SEQ}'/>)"><c:out value="${list.CATE_NAME}"/></a></li>
					</c:forEach>        
