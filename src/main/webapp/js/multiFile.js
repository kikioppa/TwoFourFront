...
function MultiSelector( list_target, max ){
	...
	this.addElement = function( element ){
		...
		if( element.tagName == 'INPUT' && element.type == 'file' ){
			...
			element.onchange = function(){
				if(fileExtCheck(this.value) && fileSizeCheck(this.id)){ // 업로드파일 체크
						//return;
					// New file input
	...
	this.addListRow = function( element ){
		...
		new_row_button.className = "btn delFile";
		...
		var file_value = element.value;		
		try{
			var file_array = file_value.split("\\");
			var file_name = file_array[ file_array.length-1 ];
			
			new_row.innerHTML = file_name;
