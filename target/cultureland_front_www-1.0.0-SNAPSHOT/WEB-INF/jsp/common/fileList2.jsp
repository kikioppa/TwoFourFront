...
<script type="text/javascript">
	...
	function fn_downFile<c:out value="${fileSn}"/>(atchFileId, fileSn){
		window.open("<c:url value='/cmm/FileDown.do?atchFileId="+atchFileId+"&fileSn="+fileSn+"'/>");
	}
	...
	function fn_deleteFile<c:out value="${fileSn}"/>(atchFileId, fileSn) {
		if ( confirm("파일을 삭제 하시겠습니까?")){
			...
			form.action = "<c:url value='/cmm/deleteFileInfs.do'/>";
			form.submit();
		}
	}
	...
	function fn_egov_check_file<c:out value="${fileSn}"/>(flag) {
		...
		<c:if test="${fn:length(fileList) > beginIndex}">
		<div class="viewDown">
			...
			<div class="fileList">
			<c:forEach var="fileVO" items="${fileList}" varStatus="status" begin="${beginIndex}">
				<c:choose>
					<c:when test="${updateFlag=='Y'}">
						...
						<img src="<c:url value='/images/ico/bu5_close.png' />" 
